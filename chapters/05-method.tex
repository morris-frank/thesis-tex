...
The idea of using SGLD in combination with deep parametrized priors for source separation was, concurrently to this work, introduced in \textcite{jayaramSource2020}. The authors empirically show that
...
\begin{enumerate}
    \item \I{All sources known and trained with prior}
    ...
    \item \I{Unstable sampling / training because of many objectives}
\end{enumerate}

\section{De-noising}
The task of de-noising is a simpler objective setting but similar to source separation. In de-noising, we want to remove i.i.d.\@ noise \(\B{\ε}\) from the observed signal \(\B{\hat{s}}\).


De-noising
\begin{marginfigure}[-15em]
    \input{figures/graphical_model_noise}%
    \caption{The graphical model for de-noising. The observed signal is generated by the sum of a noise-less signal and a parameter-less sample of Gaussian noise \(\B{\ε}\).}%
    \label{fig:graphical_model_noise}
\end{marginfigure}
