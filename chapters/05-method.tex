...
We propose the graphical model as shown in \cref{fig:graphical_model_separation} as the generative story {\color{red} I like to call it true generative model, but not sure if this is more formal the generative story} of music tracks being generated from separate source tracks. For each source, a sample is taken from the latent source distribution. The observed mix is generated deterministically from the full set of sources. Without loss of generality, we fix this function to be the mean.
...
Our stated task in \sref{ch:question} is to retreive the sources \(\{\s_1,\…,\s_N\}\) from a given mix \(\m\). Our model is trained without using sample tuples \[(\m, \{\s_1,\…,\s_N\}): f(\{\s_1,\…,\s_N\}) = \m\] which would show {\color{red} don't think 'show' is a good choice here} the relation between separate sources and mixes. The general idea is visualized in \cref{fig:method}.
...
Looking back at {\color{red} I would delete 'looking back at} the graphical model in \cref{fig:graphical_model_separation}, it implies the following factorization:
...
{\color{red} This is quite abrupt, maybe use a subsection header saying something like: Let's talk about what are the building blocks.}
...
{\color{red} as with equation 37 I think that the decoder should have N learnable parameters.}

{\color{red} Furthermore I miss the final objective for the VAE-like approach. Especially the KL terms can be used to say: "look this is how the prior interacts with the encoder/approx posterior".}

\section{Modeling the priors}
The first step in the training procedure is the training of the independent source priors \(p(\s_k)\). We have to choose a model for estimating the density that results in smooth, tight densities but also is capable enough to capture the complex data space of audio data. We choose to estimate the priors, with flow models for which we gave an overview in \sref{subsec:flows}. The models estimate a densities over the separate data sources. For both the sampling-based approach and the variational approach we need these densities to give us likelihoods of samples in that source's distribution.

We propose to adapt the FloWaveNet~\cite{kimFloWaveNet2019} architecture, as introduced in \sref{subsec:flows}. In this architecure the data splitting for the coupling layers is done along the channel dimension. As we are using mono waveforms, the squeeze operation is necessary to increase the channel depth from the one input channel. The next section will introduce the architecture more throughfully.
...
As explained in \cref{subsec:flows} {\color{red}reference is broken} the invertible flow \(f(\·)\) network directly maps the data space into a prior distribution \(p(\z)\). Through the invertibility we can directly optimize the log probability with the change of variable formula:
